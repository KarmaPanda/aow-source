...
function on_main_form_open(self)
  ...
  self.canshowsprite = false
  self.groupbox_skill.Visible = false
  bind_player(self)
  refresh_name(self)
end
...
function refresh_name(form)
  if not nx_is_valid(form.player) then
    if 0 == form.notes_space then
      ...
      gui.TextManager:Format_SetIDName("83396")
      gui.TextManager:Format_AddParam(nx_widestr(form.playername))
      self_systemcenterinfos(gui.TextManager:Format_GetText())
    end
...
function refresh_form(form, view)
  ...
  local DamageAll = view:QueryProp("DamageAll")
  local DamageMax = view:QueryProp("DamageMax")
  local FightDodge = view:QueryProp("FightDodge")
  local FightVa = view:QueryProp("FightVa")
  local FightBreakParry = view:QueryProp("FightBreakParry")
  local FightParry = view:QueryProp("FightParry")
  local FightLastTime = view:QueryProp("FightLastTime")
  local DamagePerSec = 0
  local FightMsg = view:QueryProp("FightMsg")
  local HP = view:QueryProp("FightHP")
  local MaxHP = view:QueryProp("FightMaxHP")
  local MP = view:QueryProp("FightMP")
  local MaxMP = view:QueryProp("FightMaxMP")
  local sec_FightLastTime = FightLastTime / 1000
  ...
  set_win_num(form, nx_string(FightMsg))
...
function refresh_skill(form)
  ...
  local skill_id = nx_string(form.player:QueryProp("HuashanLastSkillID"))
  if string.len(skill_id) < 3 then
    return
  end
  if form.canshowsprite then
