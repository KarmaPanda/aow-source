...
function close_form()
  local form = nx_value("form_stage_main\\form_die_guild_cross_war")
  if nx_is_valid(form) then
    form:Close()
  end
end
function on_btn_relive_guild_cross_war_click(btn)
  if not is_guild_cross_war() then
    return
  end
  nx_execute("custom_sender", "custom_relive", RELIVE_TYPE_GUILD_CROSS_WAR, nx_int(0))
end
