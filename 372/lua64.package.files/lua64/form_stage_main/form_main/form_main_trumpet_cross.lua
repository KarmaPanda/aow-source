require("form_stage_main\\form_main\\form_laba_info")
...
function change_form_size()
  ...
  local form = nx_value("form_stage_main\\form_main\\form_main_trumpet_cross")
...
function on_mltbox_info_right_click_hyperlink(self, itemindex, linkdata)
  ...
  if string.find(nx_string(linkdata), "item") ~= nil or string.find(nx_string(linkdata), "item") ~= nil then
    return
  end
  local game_client = nx_value("game_client")
  local player = game_client:GetPlayer()
