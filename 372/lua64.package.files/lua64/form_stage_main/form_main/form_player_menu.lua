...
function on_btn_add_invite_guild_click(btn)
  ...
  form:Close()
  nx_execute("custom_sender", "custom_guild_invite_member", nx_widestr(sname))
end
function on_btn_team_request_click(btn)
  local form = btn.ParentForm
  local sname = form.sender_name
  form:Close()
  nx_execute("custom_sender", "custom_team_invite", nx_string(sname))
end
function on_btn_friend_request_click(btn)
  local form = btn.ParentForm
  local sname = form.sender_name
  form:Close()
  nx_execute("form_stage_main\\form_relation\\form_relation", "interface_add_friend", nx_string(sname))
end
function on_btn_add_attention_click(btn)
  local form = btn.ParentForm
  local sname = form.sender_name
  form:Close()
  nx_execute("form_stage_main\\form_relation\\form_relation", "interface_add_attention", nx_string(sname))
end
function on_btn_add_chat_click(btn)
  local form = btn.ParentForm
  local sname = form.sender_name
  form:Close()
  nx_execute("custom_sender", "custom_request_chat", nx_widestr(sname))
end
function on_btn_add_blacklist_click(btn)
  local form = btn.ParentForm
  local sname = form.sender_name
  form:Close()
  nx_execute("form_stage_main\\form_relation\\form_relation", "interface_add_filter", nx_string(sname))
end
