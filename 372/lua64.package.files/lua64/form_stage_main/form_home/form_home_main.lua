...
function on_btn_compose_click(btn)
  ...
  if nx_is_valid(switch_manager) and not switch_manager:CheckSwitchEnable(ST_FUNCTION_FUSE_HOME) then
    local SystemCenterInfo = nx_value("SystemCenterInfo")
    if nx_is_valid(SystemCenterInfo) then
      SystemCenterInfo:ShowSystemCenterInfo(util_text("sys_home_fuse_001"), 2)
    end
    return
  end
