...
function receive_home_data(...)
  ...
  if nx_int(PlayerArgsCount) ~= nx_int(nArgsCount) then
    ...
    local WstrFeiqiDay = FormatFeiqiData(HomeFeiQiData, HomeStyle)
...
function GetHomeStyle(homestyle)
  ...
  elseif nx_int(9) == nx_int(homestyle) then
    return gui.TextManager:GetText("ui_home_fudi")
  end
end
...
function FormatFeiqiData(FeiqiDay, homestyle)
  if nx_int(FeiqiDay) < nx_int(0) then
    FeiqiDay = 0
  end
  local Maxday = nx_int(get_ini_prop("share\\Home\\HomeSystemConf.ini", "main", "over_time_day", "0"))
  if nx_int(9) == nx_int(homestyle) then
    Maxday = nx_int(get_ini_prop("share\\Home\\HomeSystemConf.ini", "main", "over_bid_day", "0"))
  end
