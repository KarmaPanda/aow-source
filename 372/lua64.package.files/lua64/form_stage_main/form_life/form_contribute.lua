...
function on_main_form_open(form)
  ...
  if nx_is_valid(databinder) then
    databinder:AddViewBind(VIEWPORT_CONTRIBUTE, form.imagegrid_contribute, "form_stage_main\\form_life\\form_contribute", "on_contribute_box_change")
  end
  form.imagegrid_contribute.typeid = VIEWPORT_CONTRIBUTE
  for i = 1, 10 do
...
function on_main_form_close(form)
  ...
  nx_execute("custom_sender", "custom_cancel_contribute")
end
...
function on_contribute_form_open(npcid)
  local form = util_get_form("form_stage_main\\form_life\\form_contribute", true)
  ...
  form.mltbox_desc:AddHtmlText(nx_widestr(gui.TextManager:GetText("ui_schoolwar_exchange_" .. npcid .. "_01")), nx_int(-1))
  util_auto_show_hide_form("form_stage_main\\form_life\\form_contribute")
end
...
function on_btn_contribute_click(btn)
  ...
  nx_execute("custom_sender", "custom_start_contribute")
end
...
function on_imagegrid_contribute_right_click(grid, index)
  local game_client = nx_value("game_client")
  local view = game_client:GetView(nx_string(VIEWPORT_CONTRIBUTE))
  if not nx_is_valid(view) then
    return
  end
  local viewobj = view:GetViewObj(nx_string(index + 1))
  if nx_is_valid(viewobj) then
    nx_execute("custom_sender", "custom_remove_contribute_item", index + 1)
  end
end
function on_imagegrid_contribute_select_changed(grid)
  ...
  if gui.GameHand.Type == GHT_VIEWITEM then
    ...
    nx_execute("custom_sender", "custom_add_contribute_item", src_viewid, src_pos, selected_index + 1)
