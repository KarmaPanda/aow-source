...
require("tips_data")
local FORM_NAME = "form_stage_main\\form_world_auction\\form_black_market"
...
function on_main_form_open(self)
  ...
  nx_execute("custom_sender", "custom_world_auction", 0, 100)
end
function on_main_form_close(self)
  ...
  nx_execute("custom_sender", "custom_world_auction", 6, 100)
  nx_destroy(self)
end
...
function on_rbtn_3_checked_changed(rbtn)
  if rbtn.Checked then
    ...
    nx_execute("custom_sender", "custom_world_auction", 2, 100)
  end
end
...
function update_auction_single(...)
  ...
  local price_add = nx_int(arg[9])
  if drop_type < nx_int(100) or drop_type > nx_int(200) then
    return
  end
  local game_client = nx_value("game_client")
  local client_player = game_client:GetPlayer()
  ...
  if nx_is_valid(gb) then
    ...
    form.gsb_auction.IsEditMode = true
    gsp_auction_add(form, auction_uid, item_config, item_amount, bind, price, competer_name, end_time, price_add)
    form.gsb_auction.IsEditMode = false
    form.gsb_auction:ResetChildrenYPos()
  end
end
...
function set_sl_auction(form, auction_uid)
  ...
  if form.sl_auction == "" then
    ...
    if nx_is_valid(gb) then
      ...
      price_min = price_min + nx_number(gb.price_add)
      form.gb_info_1.price_min = price_min
      form.gb_info_1.price_add = nx_number(gb.price_add)
      form.btn_price_down.DataSource = nx_string(gb.price_add)
      form.lbl_24.Text = util_format_string("ui_black_market_001", nx_number(gb.price_add) / 1000)
      form.lbl_23.Text = util_format_string("ui_black_market_002", nx_number(gb.price_add) / 1000)
...
function gsp_auction_add(form, auction_uid, item_config, item_amount, bind, price, competer, end_time, price_add)
  local game_client = nx_value("game_client")
  local client_player = game_client:GetPlayer()
  local self_name = client_player:QueryProp("Name")
  ...
  if nx_is_valid(gb) then
    ...
    local color_level = nx_number(get_prop_in_ItemQuery(item_config, nx_string("ColorLevel")))
    local photo = nx_execute("util_static_data", "item_query_ArtPack_by_id", item_config, "Photo")
    ig:AddItem(0, nx_string(photo), nx_widestr(item_config), nx_int(item_amount), 0)
