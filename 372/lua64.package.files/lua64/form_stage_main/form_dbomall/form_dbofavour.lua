...
function on_main_form_open(form)
  nx_execute("custom_sender", "custom_dbomall_request", DMC_SUBMSG_FAVOUR_DATAINFO)
...
function on_main_form_close(form)
  ...
  local form_dbobuy = nx_value(FORM_DBOBUY)
  if nx_is_valid(form_dbobuy) then
    form_dbobuy:Close()
  end
  nx_destroy(form)
end
...
function on_btn_buy_click(btn)
  ...
  if not nx_is_valid(cptmgr) then
    return
  end
  if not nx_find_custom(gbx, "charge_id") then
    return
  end
  ...
  local dialog = nx_execute("util_gui", "util_get_form", FORM_DBOBUY, true, false)
  if not nx_is_valid(dialog) then
    return
  end
  local config_id = nx_string(gbx.config_id)
  local photo = get_photo(config_id)
  dialog.grid_main:AddItem("", photo, "", 1, -1)
  dialog.grid_main.config_id = config_id
  dialog.lbl_name.Text = gui.TextManager:GetText(config_id)
  dialog.lbl_unitprice.Text = gui.TextManager:GetFormatText("ui_dobmall_price", nx_int64(gbx.exchange_price))
  dialog.lbl_price.Text = gui.TextManager:GetFormatText("ui_dobmall_price", nx_int64(gbx.exchange_price))
  local price_type = dbomall_manager:GetPriceType(nx_int(charge_id))
  local cur_gold = cptmgr:GetCapital(nx_int(price_type))
  dialog.lbl_gold.Text = gui.TextManager:GetFormatText("ui_dobmall_price", nx_int(cur_gold))
  dialog:Show()
  local res = nx_wait_event(100000000, dialog, "dbobuy_confirm_return")
  if res == "ok" then
    nx_execute("custom_sender", "custom_dbomall_request", DMC_SUBMSG_FAVOUR_BUYITEM, nx_int(charge_id))
  end
end
...
function on_recv_favour_datainfo(...)
  local form = nx_value("form_stage_main\\form_dbomall\\form_dbofavour")
  if not nx_is_valid(form) then
    return
  end
  local dbomall_manager = nx_value("dbomall_manager")
...
function SetChargeItemInfo(form, index, ...)
  ...
  if nx_is_valid(lbl_name) then
    lbl_name.Text = gui.TextManager:GetText(gbx.config_id)
  end
  ...
  if nx_is_valid(lbl_curprice) then
    lbl_curprice.Text = gui.TextManager:GetFormatText("ui_dobmall_new_price", nx_int(gbx.exchange_price))
  end
