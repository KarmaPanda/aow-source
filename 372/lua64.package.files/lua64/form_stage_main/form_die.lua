...
function form_die_Open(form)
  ...
  if IsInBalanceWarScene() then
    form.btn_return_city.Enabled = false
  elseif IsInWuDaoScene() or IsInLuanDouFightScene() then
    form.btn_return_city.Enabled = false
  end
  if is_in_cross_station_war() then
    form.btn_return_city.Text = nx_widestr(util_text("ui_guild_kuafu_station_shp_30"))
    form.btn_return_city.HintText = nx_widestr(util_text("ui_guild_kuafu_station_shp_31"))
  end
end
...
function on_btn_relive_local_click(btn)
  nx_execute(nx_current(), "show_ok_dialog", btn.ParentForm, RELIVE_TYPE_LOCAL)
end
  ...
  if nx_is_valid(client) then
    ...
    if nx_is_valid(player) then
      ...
      elseif twelve_dock_relive_near(player) then
        return
      elseif is_gumu_rescuing(player) then
        nx_execute("custom_sender", "custom_relive", RELIVE_TYPE_NEAR, 0)
        return
      elseif is_nlb_shimen(player) then
        nx_execute("custom_sender", "custom_relive", RELIVE_TYPE_NEAR, 0)
        return
      elseif is_ssg_probation(player) then
        return
      end
    end
...
function is_ssg_probation(player)
  if not nx_is_valid(player) then
    return false
  end
...
function is_gumu_rescuing(player)
  if not player:FindRecord("rec_gumu_rescue") then
    return false
  end
  local rows = player:GetRecordRows("rec_gumu_rescue")
  if nx_int(rows) ~= nx_int(1) then
    return false
  end
  return true
end
function is_nlb_shimen(player)
  local cur_group = player:QueryProp("NLBSchoolMeetGroup")
  if nx_int(cur_group) > nx_int(0) then
    return true
  end
  return false
end
...
function fresh_relive_form(form)
  ...
  elseif nx_int(is_jyf_faculty) == nx_int(1) then
    ...
    form.lbl_count.Text = nx_widestr(count_str)
    form.btn_relive_strong.HintText = nx_widestr(gui.TextManager:GetFormatText("sys_jysg_fh_01"))
    form.btn_relive_local.HintText = nx_widestr(gui.TextManager:GetFormatText("sys_jysg_fh_02"))
  else
    local left_count = nx_int(MAX_RELIVE_COUNT_DAILY) - nx_int(relive_count)
    local count_str = gui.TextManager:GetFormatText("ui_revive_count", nx_int(left_count))
    form.lbl_count.Text = nx_widestr(count_str)
...
function on_time_over()
  local type = RELIVE_TYPE_NEAR
  if IsInJHScene() then
    if IsInLoulan() then
      type = RELIVE_TYPE_LOULAN
    else
      type = RELIVE_TYPE_JHSCENE_HOME
    end
  end
  nx_execute("custom_sender", "custom_relive", type, nx_int(0))
end
function on_relive_check(ret)
  local form = util_get_form(get_die_form_res(), false)
...
function show_ok_dialog(form, relive_type)
  ...
  if nx_int(relive_type) == nx_int(RELIVE_TYPE_NEAR) then
    local home_uid = player:QueryProp("CurHomeUID")
    if home_uid ~= nil and home_uid ~= "" and home_uid ~= 0 then
      str = get_confirm_info(RELIVE_TYPE_HOME, nx_int(0))
    end
  end
