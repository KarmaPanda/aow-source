...
local SMALLGAME_MSG_OPEN_LOCKER_START = 1
local SMALLGAME_MSG_OPEN_LOCKER_END = 2
local SMALLGAME_MSG_LOCK_ZHEN_ROATE = 3
...
function on_main_form_open(form)
  ...
  local LockpickGame = nx_value("LockpickGame")
  ...
  form.qsgj:AddItem(0, tempphoto, nx_widestr("<font color=\"#ffffff\">" .. nx_string(MaterialNum) .. "</font>"), 0, -1)
  form.qsgj:SetItemAddInfo(nx_int(0), nx_int(1), nx_widestr("<font color=\"#ffffff\">" .. nx_string(MaterialNum) .. "</font>"))
  form.qsgj:SetItemAddInfo(nx_int(0), nx_int(2), nx_widestr(item))
  form.qsgj:ShowItemAddInfo(nx_int(0), nx_int(1), true)
  failtime = 0
...
function close_form()
  local form = nx_execute("util_gui", "util_get_form", "form_stage_main\\form_small_game\\form_game_openlocker", true)
...
function on_size_change()
  local gui = nx_value("gui")
  local form = nx_value("form_stage_main\\form_small_game\\form_game_openlocker")
...
function on_btn_quit_click(btn)
  ...
  game_visual:CustomSend(nx_int(CLIENT_CUSTOMMSG_SMAILL_GAME_MSG), nx_int(SMALLGAME_MSG_OPEN_LOCKER_END))
  close_form()
end
...
function get_point_pos(form, scene)
  ...
  local info = nx_string(nx_int(form.point1_x)) .. ":" .. nx_string(nx_int(form.point1_y)) .. ":" .. nx_string(nx_int(form.point2_x)) .. ":" .. nx_string(nx_int(form.point2_y)) .. ":" .. nx_string(nx_int(form.point3_x)) .. ":" .. nx_string(nx_int(form.point3_y)) .. ":" .. nx_string(nx_int(form.point4_x)) .. ":" .. nx_string(nx_int(form.point4_y))
end
function mouse_move(self, click_x, click_y)
  ...
  local timer = nx_value("timer_game")
  timer:UnRegister("form_stage_main\\form_small_game\\form_game_openlocker", "on_update_shake", form)
...
function on_key_down(key_value)
  ...
  while form.KeyDown do
    ...
    local LockpickGame = nx_value("LockpickGame")
    local MaxRotataPercent = math.abs(LockpickGame:GetGameRes() - form.MouseRotateAngle)
...
function game_start(home_id)
  local game_visual = nx_value("game_visual")
  if not nx_is_valid(game_visual) then
    return false
  end
  game_visual:CustomSend(nx_int(CLIENT_CUSTOMMSG_SMAILL_GAME_MSG), nx_int(SMALLGAME_MSG_OPEN_LOCKER_START), nx_string(home_id))
end
function game_send_roate(roate)
  ...
  local number = LockpickGame:GetGameRes()
  local game_visual = nx_value("game_visual")
  if not nx_is_valid(game_visual) then
    return false
  end
  game_visual:CustomSend(nx_int(CLIENT_CUSTOMMSG_SMAILL_GAME_MSG), nx_int(SMALLGAME_MSG_LOCK_ZHEN_ROATE), nx_int(roate))
end
function on_end_game(result, home_id)
  local form = nx_execute("util_gui", "util_get_form", "form_stage_main\\form_small_game\\form_game_openlocker")
  if not nx_is_valid(form) then
    return false
  end
  ...
  if result == 1 then
    ...
    if home_id ~= "" then
      enter_home_by_open_lock(form, home_id)
    end
    nx_pause(3)
  else
    if not nx_is_valid(form) then
...
function enter_home_by_open_lock(form, home_id)
  if not nx_is_valid(form) then
    return false
  end
  ...
  if is_my_home(home_id) then
    self_systemcenterinfo("home_enter_failed_03")
    return
  end
  nx_execute("form_stage_main\\form_home\\form_home_msg", "client_to_server_msg", CLIENT_SUB_ENTRY, nx_string(home_id), nx_int(enter_type_open_lock))
end
