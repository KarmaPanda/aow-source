...
local FORM_APEX_PICK = "form_stage_main\\form_taosha\\form_apex_pick"
...
function on_icg_items_select_changed(grid, index)
  ...
  local form = nx_value(FORM_APEX_PICK)
  if nx_is_valid(form) then
    form.VScrollBarValue = form.icg_items.VScrollBar.Value
  end
end
function put_item_up(grid, index)
  local gui = nx_value("gui")
  local gamehand = gui.GameHand
  local VIEW_MAIN = VIEWPORT_DROP_BOX
  local view_index = grid:GetBindIndex(index)
  nx_set_value("isSplit", gamehand.Para1)
  if not gamehand.IsDropped and gui.GameHand:IsEmpty() and not grid:IsEmpty(index) then
    local photo = grid:GetItemImage(index)
    local amount = grid:GetItemNumber(index)
    gui.GameHand:SetHand("viewitem", nx_string(photo), nx_string(VIEW_MAIN), nx_string(view_index), nx_string(amount), "")
  end
end
