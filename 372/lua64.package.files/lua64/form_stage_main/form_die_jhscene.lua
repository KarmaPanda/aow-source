...
function form_die_Open(form)
  ...
  form.select_type = RELIVE_TYPE_JHSCENE_HOME
...
function on_btn_return_city_click(btn)
  nx_execute(nx_current(), "show_ok_dialog", btn.ParentForm, RELIVE_TYPE_JHSCENE_HOME)
end
...
function fresh_relive_form(form)
  ...
  if player:FindRecord("JHDropBoxRec") and 0 < nx_number(player:GetRecordRows("JHDropBoxRec")) then
...
function on_time_over()
  nx_execute("custom_sender", "custom_relive", RELIVE_TYPE_JHSCENE_HOME, nx_int(0))
end
