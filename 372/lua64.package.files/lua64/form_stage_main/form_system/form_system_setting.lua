...
function main_form_open(self)
  ...
  self.is_low_shadow_used = game_config:IsLowShadowUsed()
  local switch_manager = nx_value("SwitchManager")
  if nx_is_valid(switch_manager) and switch_manager:CheckSwitchEnable(ST_FUNCTION_CHECK_FPS_LOW_SHADOW) then
    game_config:StopCheckFps()
  end
...
function on_cancel_btn_click(self)
  ...
  if nx_is_valid(switch_manager) and switch_manager:CheckSwitchEnable(ST_FUNCTION_CHECK_FPS_LOW_SHADOW) then
    local form = self.ParentForm
    if form.is_low_shadow_used then
      game_config:OpenLowShadow()
    end
    game_config:StartCheckFps()
  end
  close_form(self.ParentForm)
