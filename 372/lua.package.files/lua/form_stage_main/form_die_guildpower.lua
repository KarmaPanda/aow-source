...
local FORM_PATH = "form_stage_main\\form_die_guildpower"
...
function on_main_form_open(form)
  ...
  form.revert = os.time() + 10
  form.LuaScript = nx_current()
  local asynor = nx_value("common_execute")
  asynor:AddExecute("relive_fresh_timer", form, nx_float(1))
...
function close_form()
  local form = util_get_form(FORM_PATH, false)
  if nx_is_valid(form) then
    form:Close()
  end
end
...
function on_btn_relive_guild_powerwar_click(btn)
  nx_execute("custom_sender", "custom_relive", RELIVE_TYPE_POWER)
end
