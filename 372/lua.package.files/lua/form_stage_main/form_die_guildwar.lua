...
function init_form(form)
  ...
  for i = 1, RELIVE_POINT_COUNT do
    local btn_relive = form.gbox_guild_map:Find("rbtn_relive_" .. i)
  ...
  for i = 1, RELIVE_POINT_COUNT do
    ...
    local temp_canuse = sub_table.canuse
    if temp_warside == player_side then
      local temp_rbtn = form.gbox_guild_map:Find(nx_string("rbtn_relive_") .. nx_string(i))
      if nx_is_valid(temp_rbtn) then
        temp_rbtn.can_use = temp_canuse
        temp_rbtn.relive_index = temp_index
        temp_rbtn.Visible = true
        temp_rbtn.Enabled = temp_canuse == 1
      end
    end
  end
  local rbtn_index = -1
...
function on_rbtn_relive_checked_changed(self)
  ...
  if self.Checked then
    ...
    form.can_use = self.can_use
  end
  if not nx_number(form.relive_index) ~= -1 then
    form.btn_relive_point.Enabled = true
  else
    form.btn_relive_point.Enabled = false
  end
end
function on_rbtn_relive_left_double_click(self)
  ...
  local cur_can_use = self.can_use
  local cur_relive_index = self.relive_index
  if nx_int(cur_can_use) ~= nx_int(1) then
    return 0
  end
...
function on_btn_relive_point_click(self)
  ...
  if nx_number(cur_relive_index) < nx_number(0) then
    form.lbl_relive_name.Text = gui.TextManager:GetText("ui_guild_war_map_fuhuodian_nochoose")
    return 0
  end
  if not show_ok_dialog(RELIVE_TYPE_GUILD, nx_int(0), nx_number(cur_relive_index)) then
...
function on_rbtn_relive_get_capture(self)
  ...
  form.lbl_relive_name.Text = gui.TextManager:GetText("ui_guild_war_relivepoint_name_" .. nx_string(self.relive_index))
  form.lbl_relive_state.Text = gui.TextManager:GetText("ui_guild_war_relivepoint_state_" .. self.can_use)
end
function on_rbtn_relive_lost_capture(self)
  ...
  if nx_number(form.relive_index) ~= -1 then
    form.lbl_relive_name.Text = gui.TextManager:GetText("ui_guild_war_relivepoint_name_" .. nx_string(form.relive_index))
    form.lbl_relive_state.Text = gui.TextManager:GetText("ui_guild_war_relivepoint_state_" .. nx_string(form.can_use))
  else
...
function show_guild_war_relive_form(relivepoints)
  local gui = nx_value("gui")
  local form = nx_execute("util_gui", "util_get_form", FORM_DIE_GUILDWAR, true)
  if not nx_is_valid(form) then
    return 0
  end
  form.relive_index = -1
  ...
  nx_execute("util_gui", "util_show_form", FORM_DIE_GUILDWAR, true)
  local common_execute = nx_value("common_execute")
