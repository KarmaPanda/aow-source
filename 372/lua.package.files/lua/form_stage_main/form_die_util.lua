...
RELIVE_TYPE_FLEE_IN_NIGHT = 50
RELIVE_TYPE_ROGUE = 51
RELIVE_TYPE_MAX = 52
...
function get_die_form_res()
  ...
  if IsInFleeInNight() then
    return "form_stage_main\\form_die_flee_in_night"
  end
  if IsInRogue() then
    return "form_stage_main\\form_die_rogue"
  end
...
function close_dead_form()
  ...
  local form_die_flee_in_night = nx_value("form_stage_main\\form_die_flee_in_night")
  if nx_is_valid(form_die_flee_in_night) then
    form_die_flee_in_night.Visible = false
    form_die_flee_in_night:Close()
  end
  local form_die_roguet = nx_value("form_stage_main\\form_die_rogue")
  if nx_is_valid(form_die_roguet) then
    form_die_roguet.Visible = false
    form_die_roguet:Close()
  end
end
...
function IsInRogue()
  ...
  if interactmgr:GetInteractStatus(ITT_ROGUE) == PIS_IN_GAME then
    return true
  end
  return false
end
...
function IsInKof()
  return nx_execute("form_stage_main\\form_kof\\kof_util", "is_in_kof")
end
