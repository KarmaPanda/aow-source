...
local FORM_NAME = "form_stage_main\\form_force\\form_force_gumupai_lot"
...
function on_rbtn_checked_changed(rbtn)
  if rbtn.Checked == true then
    nx_execute("custom_sender", "custom_ancient_tomb_sender", 21, nx_string(rbtn.answer))
  end
end
function open_form(...)
...
function on_update_time(form)
  if nx_int(form.pbar_1.Value) <= nx_int(0) then
    close_form()
    return
  end
  form.pbar_1.Value = form.pbar_1.Value - 10
end
