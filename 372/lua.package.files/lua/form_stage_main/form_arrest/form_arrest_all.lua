...
function on_main_form_open(form)
  ...
  unit_money, prison_div, max_prison, be_wanted_rate = nx_execute("form_stage_main\\form_arrest\\form_arrest_define", "get_arrest_receive_need_info")
  return 1
end
...
function show_all_arrest_info(...)
  local form = nx_execute("util_gui", "util_get_form", "form_stage_main\\form_arrest\\form_arrest_all", true, false)
  ...
  form.textgrid_list:SetColWidth(4, 0)
  local gui = nx_value("gui")
  ...
  form.page_index = 1
  util_show_form("form_stage_main\\form_arrest\\form_arrest_all", true)
end
function show_detailed_info(...)
  ...
  form.lbl_num.Text = nx_widestr(arg[17]) .. nx_widestr(util_text("ui_ren"))
  ...
  form.lbl_times.Text = nx_widestr(nx_int(arg[12] * be_wanted_rate / unit_money)) .. nx_widestr(util_text("ui_ci"))
...
function on_btn_next_click(btn)
  ...
  if form.page_index == 1 then
    ...
    game_visual:CustomSend(nx_int(CLIENT_CUSTOMMSG_ARREST_WARRANT), nx_int(ARREST_CUSTOMMSG_QUERY_ARREST), nx_int(arrest_detail_all), nx_string(form.uid))
  end
...
function on_btn_danger_click(btn)
  ...
  sort_type = 0
  game_visual:CustomSend(nx_int(CLIENT_CUSTOMMSG_ARREST_WARRANT), nx_int(ARREST_CUSTOMMSG_QUERY_BY_SORT), nx_int(1), nx_int(5), nx_int(arrest_tocustom_show_query), nx_int(0))
  return
end
function on_btn_money_click(btn)
  ...
  sort_type = 1
  game_visual:CustomSend(nx_int(CLIENT_CUSTOMMSG_ARREST_WARRANT), nx_int(ARREST_CUSTOMMSG_QUERY_BY_SORT), nx_int(1), nx_int(5), nx_int(arrest_tocustom_show_query), nx_int(1))
  return
end
function on_btn_time_click(btn)
  ...
  sort_type = 2
  game_visual:CustomSend(nx_int(CLIENT_CUSTOMMSG_ARREST_WARRANT), nx_int(ARREST_CUSTOMMSG_QUERY_BY_SORT), nx_int(1), nx_int(5), nx_int(arrest_tocustom_show_query), nx_int(2))
  return
end
function on_btn_left_click(btn)
  ...
  game_visual:CustomSend(nx_int(CLIENT_CUSTOMMSG_ARREST_WARRANT), nx_int(ARREST_CUSTOMMSG_QUERY_BY_SORT), nx_int(form.grid_index - 1), nx_int(5), nx_int(arrest_tocustom_show_query), sort_type)
  return
end
...
function on_btn_first_click(btn)
  ...
  game_visual:CustomSend(nx_int(CLIENT_CUSTOMMSG_ARREST_WARRANT), nx_int(ARREST_CUSTOMMSG_QUERY_BY_SORT), nx_int(1), nx_int(5), nx_int(arrest_tocustom_show_query), sort_type)
  return
end
...
function on_btn_search_click(btn)
  ...
  game_visual:CustomSend(nx_int(CLIENT_CUSTOMMSG_ARREST_WARRANT), nx_int(ARREST_CUSTOMMSG_SEARCH_ARREST), nx_widestr(form.ipt_name.Text), nx_int(arrest_tocustom_show_query))
  return
end
