...
function on_buy_click(btn)
  ...
  if not show_confirm_info("buy_charge_item_2", form.config, nums, str) then
    return
  end
  ...
  if nx_is_valid(game_visual) then
    game_visual:CustomSend(nx_int(CLIENT_CUSTOMMSG_CHARGE_SHOP), nx_int(3), charge_id, nums)
    form:Close()
  end
end
