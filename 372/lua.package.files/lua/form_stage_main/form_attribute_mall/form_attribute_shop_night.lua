...
local FORM_ATTRIBUTE_MALL_NIGHT = "form_stage_main\\form_attribute_mall\\form_attribute_shop_night"
...
function on_main_form_open(form)
  ...
  form.btn_shouqi.Visible = false
  local databinder = nx_value("data_binder")
  databinder:AddRolePropertyBind("DreamCurrency", "int", form, FORM_ATTRIBUTE_MALL_NIGHT, "on_DreamCurrency_changed")
  databinder:AddRolePropertyBind("lonelynight", "int", form, FORM_ATTRIBUTE_MALL_NIGHT, "on_lonelynight_changed")
  databinder:AddRolePropertyBind("nfeightschoolPoints", "int", form, FORM_ATTRIBUTE_MALL_NIGHT, "on_nfeightschoolPoints_changed")
  databinder:AddRolePropertyBind("FiveSkyTotal", "int", form, FORM_ATTRIBUTE_MALL_NIGHT, "on_FiveSkyTotal_changed")
  databinder:AddRolePropertyBind("FIN_PropPoint", "int", form, FORM_ATTRIBUTE_MALL_NIGHT, "on_FIN_PropPoint_changed")
  databinder:AddRolePropertyBind("GhostCityQBValue", "int", form, FORM_ATTRIBUTE_MALL_NIGHT, "on_GhostCityQBValue_changed")
  databinder:AddRolePropertyBind("GhostCityCLValue", "int", form, FORM_ATTRIBUTE_MALL_NIGHT, "on_GhostCityCLValue_changed")
  databinder:AddRolePropertyBind("TianqiPoints", "int", form, FORM_ATTRIBUTE_MALL_NIGHT, "on_TianqiPoints_changed")
end
function on_main_form_close(form)
  local databinder = nx_value("data_binder")
  if nx_is_valid(databinder) then
    databinder:DelRolePropertyBind("DreamCurrency", form)
    databinder:DelRolePropertyBind("lonelynight", form)
    databinder:DelRolePropertyBind("nfeightschoolPoints", form)
    databinder:DelRolePropertyBind("FiveSkyTotal", form)
    databinder:DelRolePropertyBind("FIN_PropPoint", form)
    databinder:DelRolePropertyBind("GhostCityQBValue", form)
    databinder:DelRolePropertyBind("GhostCityCLValue", form)
    databinder:DelRolePropertyBind("TianqiPoints", form)
  end
  nx_execute("form_stage_main\\form_night_forever\\form_night_forever", "hide_more")
  nx_destroy(form)
end
function open_form()
  local form = util_get_form(FORM_ATTRIBUTE_MALL_NIGHT, true)
...
function close_form()
  local form = util_get_form(FORM_ATTRIBUTE_MALL_NIGHT, false, false)
  if nx_is_valid(form) then
    form:Close()
  end
end
...
function on_rbtn_sub_checked_changed(rbtn)
  ...
  if rbtn.Checked then
    ...
    show_prop(form, nx_number(sub_index))
  end
end
function on_btn_zhankai_click(btn)
  local form = btn.ParentForm
  local form_huobi = util_get_form("form_stage_main\\form_attribute_mall\\form_attribute_huobi", true, false)
  if not nx_is_valid(form_huobi) then
    return
  end
  nx_execute("form_stage_main\\form_night_forever\\form_night_forever", "show_more", form_huobi)
  form.btn_zhankai.Visible = false
  form.btn_shouqi.Visible = true
end
function on_btn_shouqi_click(btn)
  local form = btn.ParentForm
  nx_execute("form_stage_main\\form_night_forever\\form_night_forever", "hide_more")
  form.btn_shouqi.Visible = false
  form.btn_zhankai.Visible = true
end
function show_prop(form, sub_index)
  form.lbl_5.Visible = false
  form.lbl_DreamCurrency.Visible = false
  form.lbl_9.Visible = false
  form.lbl_lonelynight.Visible = false
  form.lbl_17.Visible = false
  form.lbl_nfeightschoolPoints.Visible = false
  form.lbl_19.Visible = false
  form.lbl_FiveSkyTotal.Visible = false
  form.lbl_22.Visible = false
  form.lbl_FIN_PropPoint.Visible = false
  form.lbl_8.Visible = false
  form.lbl_GhostCityQBValue.Visible = false
  form.lbl_18.Visible = false
  form.lbl_GhostCityCLValue.Visible = false
  form.lbl_16.Visible = false
  form.lbl_TianqiPoints.Visible = false
  if sub_index == 8 then
    form.lbl_5.Visible = true
    form.lbl_DreamCurrency.Visible = true
  elseif sub_index == 9 then
    form.lbl_9.Visible = true
    form.lbl_lonelynight.Visible = true
  elseif sub_index == 10 then
    form.lbl_17.Visible = true
    form.lbl_nfeightschoolPoints.Visible = true
  elseif sub_index == 11 then
    form.lbl_22.Visible = true
    form.lbl_FIN_PropPoint.Visible = true
  elseif sub_index == 12 then
    form.lbl_19.Visible = true
    form.lbl_FiveSkyTotal.Visible = true
  elseif sub_index == 53 then
    form.lbl_8.Visible = true
    form.lbl_GhostCityQBValue.Visible = true
  elseif sub_index == 54 then
    form.lbl_18.Visible = true
    form.lbl_GhostCityCLValue.Visible = true
  elseif sub_index == 55 then
    form.lbl_16.Visible = true
    form.lbl_TianqiPoints.Visible = true
  end
end
...
function on_GhostCityQBValue_changed(form)
  ...
  local value = client_player:QueryProp("GhostCityQBValue")
  if nx_number(value) < 0 then
    value = 0
  end
  local max_value = get_ini_prop_maxvalue("GhostCityQBValue")
  form.lbl_GhostCityQBValue.Text = nx_widestr(value) .. nx_widestr("/") .. nx_widestr(max_value)
end
function on_GhostCityCLValue_changed(form)
  local game_client = nx_value("game_client")
  local client_player = game_client:GetPlayer()
  if not nx_is_valid(client_player) then
    return
  end
  local value = client_player:QueryProp("GhostCityCLValue")
  if nx_number(value) < 0 then
    value = 0
  end
  local max_value = get_ini_prop_maxvalue("GhostCityCLValue")
  form.lbl_GhostCityCLValue.Text = nx_widestr(value) .. nx_widestr("/") .. nx_widestr(max_value)
end
function on_TianqiPoints_changed(form)
  local game_client = nx_value("game_client")
  local client_player = game_client:GetPlayer()
  if not nx_is_valid(client_player) then
    return
  end
  local value = client_player:QueryProp("TianqiPoints")
  if nx_number(value) < 0 then
    value = 0
  end
  local max_value = get_ini_prop_maxvalue("TianqiPoints")
  form.lbl_TianqiPoints.Text = nx_widestr(value) .. nx_widestr("/") .. nx_widestr(max_value)
end
