...
local FORM_DKP_1 = "form_stage_main\\form_relation\\form_relation_guild\\form_new_guild_dkp_1"
local FORM_DKP_2 = "form_stage_main\\form_relation\\form_relation_guild\\form_new_guild_dkp_2"
...
function on_textgrid_dkp_right_select_grid(grid, row, col)
  local form = grid.ParentForm
  if not nx_is_valid(form) then
    return 0
  end
  ...
  if nx_is_valid(gui) then
    ...
    form.groupbox_1.AbsLeft = x
    form.groupbox_1.AbsTop = y
  end
end
function on_btn_clear_click(btn)
  ...
  local lengh = nx_ws_length(form.select_name)
  ...
  if not show_confirm_info("19657", nx_widestr(form.select_name)) then
    return 0
  end
  local from = (nx_int(form.pageno) - 1) * 10
  local to = form.pageno * 10
  game_visual:CustomSend(nx_int(CLIENT_CUSTOMMSG_GUILD), nx_int(SUB_CUSTOMMSG_DKP_RESET), nx_widestr(form.select_name), nx_int(from), nx_int(to))
  request_dkp_list(form.pageno)
end
...
function on_btn_modify_click(btn)
  ...
  form.groupbox_1.Visible = false
  util_auto_show_hide_form(FORM_DKP_1)
end
...
function on_btn_team_click(btn)
  util_auto_show_hide_form(FORM_DKP_2)
end
...
function on_btn_calculate_one_click(btn)
  ...
  local from = (nx_int(form.pageno) - 1) * 10
  local to = form.pageno * 10
  game_visual:CustomSend(nx_int(CLIENT_CUSTOMMSG_GUILD), nx_int(SUB_CUSTOMMSG_DKP_CALCULATE_PERSONAL), nx_widestr(form.select_name), nx_int(from), nx_int(to))
end
