...
local g_form_name = "form_stage_main\\form_consign\\form_buy_capital_bind"
...
function on_main_form_open(form)
  ...
  ret = databinder:AddRolePropertyBind("CapitalType4", "int", form, g_form_name, "on_card_changed")
...
function on_main_form_close(form)
  ...
  if nx_is_valid(binder) then
    ...
    binder:DelRolePropertyBind("CapitalType4", form)
  end
  nx_destroy(form)
end
...
function on_combobox_choose_selected(cbox)
  ...
  local v = mgr:FormatCapital(nx_int(CAPITAL_TYPE_BIND_CARD), nx_int64(nums))
  cbox.ParentForm.mltbox_silver.HtmlText = v
  form.buy_value = nums
  form.btn_2.Enabled = nx_number(wstr) <= form.gold and nx_int(nums) <= nx_int(form.max_buy_value)
end
function on_ok_click(btn)
  ...
  local mgr = form.CapitalMgr
  if show_confirm_info("ui_buy_yinpiao_tips", mgr:FormatCapital(CAPITAL_TYPE_BIND_CARD, nx_int64(buy_value)), mgr:FormatCapital(CAPITAL_TYPE_GOLDEN, nx_int64(consum_point))) then
    nx_execute("custom_sender", "custom_buy_capital", 8, buy_value)
  end
end
...
function on_card_changed(form)
  local card = form.CapitalMgr:GetCapital(CAPITAL_TYPE_BIND_CARD)
