...
local FORM_NAME = "form_stage_main\\form_taosha\\form_apex_notice"
local CTS_SUB_APEX_MSG_QUIT = 4
function open_form(...)
  local form = get_form()
...
function on_main_form_open(form)
  ...
  if nx_is_valid(databinder) then
    databinder:AddRolePropertyBind("Dead", "int", form, FORM_NAME, "dead_changed")
  end
end
...
function on_btn_quit_click(btn)
  ...
  if ok then
    nx_execute("custom_sender", "custom_apex", nx_int(CTS_SUB_APEX_MSG_QUIT))
    close_form()
  end
end
function on_btn_stopsee_click(btn)
  nx_execute("form_stage_main\\form_taosha\\form_apex_awards", "request_open_ui")
  nx_execute("form_stage_main\\form_taosha\\apex_util", "clear_see_state")
  btn.Visible = false
end
...
function get_step_name(war_step, sub_step)
  ...
  elseif war_step == nx_int(2) then
    if sub_step == nx_int(1) then
      tip_id = "ui_apex_fight_001"
    elseif sub_step == nx_int(2) then
      tip_id = "ui_apex_fight_002"
    end
  elseif war_step == nx_int(3) then
...
function show_special_event(event_index, event_id, event_remain_time)
  ...
  local shot_text = util_format_string(nx_string("ui_apex_event_") .. nx_string(event_id))
  local long_text = util_format_string(nx_string("tip_apex_event_") .. nx_string(event_id))
