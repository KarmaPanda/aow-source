...
function on_recv_data(...)
  local form = nx_value("form_test\\form_move_check")
  if not nx_is_valid(form) then
    return
  end
  local dif = arg[1]
  ...
  if 0 < sum_time then
    form.fipt_average.Text = nx_widestr(total / sum_time)
  end
  form.fipt_check_total.Text = nx_widestr(check_total)
  form.fipt_check_total_time.Text = nx_widestr(check_sum_time)
  if 0 < check_sum_time then
    form.fipt_check_total_average.Text = nx_widestr(check_total / check_sum_time)
  end
