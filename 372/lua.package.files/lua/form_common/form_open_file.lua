function util_open_file(ext, path)
...
function util_open_file_with_abspath(ext, open_path)
  local form = nx_value(nx_current())
  if not nx_is_valid(form) then
    form = nx_execute("util_gui", "util_get_form", nx_current(), true, false)
    if not nx_is_valid(form) then
...
function on_main_form_close(self)
  nx_gen_event(self, "open_file_return", "cancel")
...
function ok_btn_click(self)
  ...
  if "" == path then
    nx_gen_event(form, "open_file_return", "cancel")
  else
    nx_gen_event(form, "open_file_return", "ok", path, form.file_name_ext)
  end
  form:Close()
end
