...
function on_btn_ok_click(btn)
  ...
  local file_name = nx_string(form.ipt_filename.Text)
  if 0 == string.len(file_name) then
    return
  end
  local file_search = form.file_search
  local file_exist = file_search:SearchFile(form.path, file_name .. ".face")
  if file_exist then
    local res = show_confirm(gui.TextManager:GetText("ui_face_cover_confirm"))
    if "cancel" == res then
      return
    end
  end
  local full_name = form.path .. file_name .. ".face"
  nx_gen_event(form, "save_face_return", "ok", full_name)
  form:Close()
end
