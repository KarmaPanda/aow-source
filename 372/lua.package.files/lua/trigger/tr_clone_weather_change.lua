...
function async_set_sky_sun(trigger, ini, append_path)
  ...
  if nx_boolean(weather.show_sun) == true then
    sun.ShowFlare = nx_boolean(ini:ReadString("weather", "ShowFlare", nx_string(weather.show_flare)))
    sun.ShowGlow = true
...
function async_set_weather(trigger, ini, append_path)
  ...
  local sky = scene.sky
  local weather = scene.Weather
  weather.FogLinear = nx_boolean(ini:ReadString("weather", "FogLinear", nx_string(weather.FogLinear)))
  weather.FogExp = nx_boolean(ini:ReadString("weather", "FogExp", nx_string(weather.FogExp)))
  weather.FogStart = nx_float(ini:ReadFloat("weather", "FogStart", weather.FogStart))
  weather.FogEnd = nx_float(ini:ReadFloat("weather", "FogEnd", weather.FogEnd))
  weather.FogHeight = nx_float(ini:ReadFloat("weather", "FogHeight", weather.FogHeight))
  weather.FogDensity = nx_float(ini:ReadFloat("weather", "FogDensity", weather.FogDensity))
  ...
  weather.WindSpeed = nx_float(ini:ReadFloat("weather", "WindSpeed", weather.WindSpeed))
  weather.WindAngle = nx_float(ini:ReadFloat("weather", "WindAngle", weather.WindAngle))
...
function set_weather(trigger, file_name, weather)
  local scene = nx_value("game_scene")
  local sky = scene.sky
  local sun = scene.sun
  if not nx_is_valid(scene) then
  ...
  if weather ~= nil and nx_string(weather) ~= nx_string("0") and nx_string(weather) ~= nx_string("") then
    ini.FileName = nx_resource_path() .. nx_string(weather)
  else
...
function get_weather_file()
  local weather_file = "map\\ter\\" .. nx_string(get_clone_res()) .. "\\weather" .. nx_string(get_clone_lvl()) .. ".ini"
  return nx_string(weather_file)
end
function get_defualt_weather()
  local weather_file = "map\\ter\\" .. nx_string(get_clone_res()) .. "\\weather.ini"
  return nx_string(weather_file)
end
function change_volumelighting()
  local scene = nx_value("game_scene")
  if not nx_is_valid(scene) then
    return
  end
  ...
  local enable = ppvolumelighting_uiparam.enable
  if not enable then
    return
  end
  local filepath = nx_resource_path() .. "map\\ter\\" .. nx_string(get_clone_res()) .. "\\ppvolumelighting" .. nx_string(get_clone_lvl()) .. ".ini"
  nx_call("terrain\\volumelighting", "load_ppvolumelighting_ini", scene, filepath)
