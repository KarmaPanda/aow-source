...
local FORM_AUTHENTICATION = "form_stage_main\\form_relation\\form_relation_guild\\form_new_guild_authentication"
...
local achieve_lbl_back_pix1 = 38
local achieve_lbl_back_pix2 = 40
local achieve_rank_interval = 2
local achieve_quanfu_height = 0
...
function on_main_form_open(self)
  ...
  self.rbtn_member_auth.Checked = true
  request_captain_Auth_list()
  request_auth_list(self.pageno)
...
function on_recv_auth_list(from, to, rowcount, ...)
  local form = nx_value("form_stage_main\\form_relation\\form_relation_guild\\form_new_guild_authentication")
  if not nx_is_valid(form) then
    return 0
  end
...
function show_game_step_auth_info(form, auth_info)
  ...
  for i = 1, section_count do
    ...
    if nx_is_valid(groupbox_one) then
      ...
      if not nx_is_valid(lbl_auth_name1) then
        return
      end
