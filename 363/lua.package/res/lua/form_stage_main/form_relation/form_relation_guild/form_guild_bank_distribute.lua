...
function on_main_form_close(form)
  ...
  nx_destroy(form)
  nx_set_value("form_stage_main\\form_relation\\form_relation_guild\\form_guild_bank_distribute", nx_null())
end
...
function on_btn_cancle_click(btn)
  ...
  if res == "ok" then
    local dis_data = form.dis_data
    dis_data:ClearChild()
    local main_form = nx_value("form_stage_main\\form_relation\\form_relation_guild\\form_guild_bank")
...
function send_request_distribute(form)
  ...
  for i = 0, count do
    ...
    if nx_is_valid(child) then
      game_visual:CustomSend(nx_int(CLIENT_CUSTOMMSG_GUILDBUILDING), nx_int(CLIENT_SUBMSG_REQUEST_DISTRIBUTE), form.npcid, nx_int(i), nx_int(count - 1), nx_widestr(child.name), nx_int(child.dis_num))
    end
  end
end
function on_recv_member_list(all_member_num, active_member_num, current_siliver_num, from, to, ...)
  local form = nx_value("form_stage_main\\form_relation\\form_relation_guild\\form_guild_bank_distribute")
  if not nx_is_valid(form) then
    return 0
  end
