...
local WE_POS_INDEX_NONE = 0
local WE_POS_INDEX_BLADE = 1
local WE_POS_INDEX_SWORD = 2
local WE_POS_INDEX_THORN = 3
local WE_POS_INDEX_SBLADE = 4
local WE_POS_INDEX_SSWORD = 5
local WE_POS_INDEX_STHORN = 6
local WE_POS_INDEX_STUFF = 7
local WE_POS_INDEX_COSH = 8
local WE_POS_INDEX_BLADE_CARD = 9
local WE_POS_INDEX_SWORD_CARD = 10
local WE_POS_INDEX_THORN_CARD = 11
local WE_POS_INDEX_SBLADE_CARD = 12
local WE_POS_INDEX_SSWORD_CARD = 13
local WE_POS_INDEX_STHORN_CARD = 14
local WE_POS_INDEX_STUFF_CARD = 15
local WE_POS_INDEX_COSH_CARD = 16
local WE_POS_INDEX_MAX = 17
local MAP_INDEX_WEAPON = {
  [ITEMTYPE_WEAPON_BLADE] = WE_POS_INDEX_BLADE,
  [ITEMTYPE_WEAPON_SWORD] = WE_POS_INDEX_SWORD,
  [ITEMTYPE_WEAPON_THORN] = WE_POS_INDEX_THORN,
  [ITEMTYPE_WEAPON_SBLADE] = WE_POS_INDEX_SBLADE,
  [ITEMTYPE_WEAPON_SSWORD] = WE_POS_INDEX_SSWORD,
  [ITEMTYPE_WEAPON_STHORN] = WE_POS_INDEX_STHORN,
  [ITEMTYPE_WEAPON_STUFF] = WE_POS_INDEX_STUFF,
  [ITEMTYPE_WEAPON_COSH] = WE_POS_INDEX_COSH
}
...
function on_main_form_open(form)
  ...
  databinder:AddViewBind(VIEWPORT_WEAPON_HANGER_BOX, form, "form_stage_main\\form_home\\form_weapon_hanger", "on_weapon_hanger_viewport_changed")
end
...
function on_weapon_hanger_viewport_changed(form, optype, view_ident, index)
  ...
  if optype == "createview" then
    ...
    for i = 1, count do
      ...
      if nx_is_valid(item_obj) then
        ...
        if nx_number(pos_index) > WE_POS_INDEX_NONE and nx_number(pos_index) < WE_POS_INDEX_MAX then
...
function on_imagegrid_select_changed(grid, index)
  ...
  if not gui.GameHand:IsEmpty() and gui.GameHand.Type == GHT_VIEWITEM then
    ...
    game_visual:CustomSend(nx_int(CLIENT_CUSTOMMSG_FURNITURE), 2, nx_int(src_viewid), nx_int(src_pos), nx_int(grid.DataSource))
...
function on_imagegrid_rightclick_grid(grid, index)
  ...
  game_visual:CustomSend(nx_int(CLIENT_CUSTOMMSG_FURNITURE), 3, nx_int(grid.DataSource))
end
function on_imagegrid_mousein_grid(grid, index)
  ...
  local game_client = nx_value("game_client")
  local view = game_client:GetView(nx_string(VIEWPORT_WEAPON_HANGER_BOX))
  if not nx_is_valid(view) then
    return
  end
...
function IsMatchedPos(item, pos)
  ...
  if nx_number(pos) <= WE_POS_INDEX_NONE or nx_number(pos) >= WE_POS_INDEX_MAX then
    return false
  end
  local pos_index = GetMatchedPos(item)
  if nx_number(pos_index) ~= nx_number(pos) and nx_number(pos_index) ~= nx_number(WE_POS_INDEX_MAX) then
    return false
  end
  return true
end
function GetMatchedPos(item)
  ...
  if nx_string(script) == nx_string("Equipment") then
    ...
    if nx_string(equip_type) ~= nx_string("Weapon") then
      return WE_POS_INDEX_NONE
    end
    local item_type = nx_int(item:QueryProp("ItemType"))
    local pos_index = GetIndexValue(item_type)
    if nx_number(pos_index) < nx_number(WE_POS_INDEX_BLADE) or nx_number(pos_index) > nx_number(WE_POS_INDEX_COSH) then
      return WE_POS_INDEX_NONE
    end
    return pos_index
  elseif nx_string(script) == nx_string("CardItem") then
    ...
    local item_type = nx_int(card_info_table[4])
    local pos_index = GetIndexValue(item_type)
    if nx_number(pos_index) < nx_number(WE_POS_INDEX_BLADE) or nx_number(pos_index) > nx_number(WE_POS_INDEX_COSH) then
      return WE_POS_INDEX_NONE
    end
    return pos_index + WE_POS_INDEX_COSH
  end
  return WE_POS_INDEX_NONE
end
function GetIndexValue(weapon_type)
  if nx_number(weapon_type) < ITEMTYPE_WEAPON_BLADE or nx_number(weapon_type) > ITEMTYPE_WEAPON_COSH then
    return WE_POS_INDEX_NONE
  end
  local index = MAP_INDEX_WEAPON[nx_number(weapon_type)]
  if index == nil then
    return WE_POS_INDEX_NONE
  end
  return index
