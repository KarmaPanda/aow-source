...
function bind_call_back(form)
  ...
  if nx_is_valid(databinder) then
    databinder:AddRolePropertyBind("SnDaNetBarDayPrize", "int", form, "form_stage_main\\form_netbar\\form_netbar_snda", "on_netbar_changed")
  end
end
...
function set_netbar_info(form)
  ...
  local right_level = client_player:QueryProp("SnDaNetBarRight")
...
function show_silver_info(form, right_level)
  ...
  local last_date = client_player:QueryProp("SnDaNetBarDayPrize")
...
function on_btn_day_click(btn)
  ...
  game_visual:CustomSend(nx_int(CLIENT_CUSTOMMSG_NETBAR_GET_RPIZE), 1)
end
