...
local FORM_NAME = "form_stage_main\\form_match\\form_guildunion_timer"
...
function check_notice_reach_score(form)
  if not (nx_find_custom(form, "zq_old_score") and nx_find_custom(form, "hq_old_score") and nx_find_custom(form, "zq_score")) or not nx_find_custom(form, "hq_score") then
    return
  end
  local var = get_notice_scores()
  local score_list = util_split_string(var, ",")
  for i = 1, table.getn(score_list) do
    if nx_int(form.zq_old_score) >= nx_int(score_list[i]) or nx_int(form.hq_old_score) >= nx_int(score_list[i]) then
      return
    end
