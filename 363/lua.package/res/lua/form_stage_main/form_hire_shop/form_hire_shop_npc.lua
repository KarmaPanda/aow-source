...
function on_main_form_close(form)
  nx_destroy(form)
  nx_set_value("form_stage_main\\form_hire_shop\\form_hire_shop_npc", nx_null())
end
function reset_grid_data(form)
  ...
  form.grid_style_info:SetColTitle(0, nx_widestr(gui.TextManager:GetText("ui_hire_shop_npc_desc")))
  form.grid_style_info:SetColTitle(1, nx_widestr(gui.TextManager:GetText("ui_hire_shop_npc_golden")))
  form.grid_style_info:SetColTitle(2, nx_widestr(gui.TextManager:GetText("ui_hire_shop_npc_silver")))
  local ini = nx_execute("util_functions", "get_ini", "share\\Trade\\HireShopEmployNpc.ini")
  ...
  for i = 1, sec_size do
    ...
    local config_id = ini:ReadString(i - 1, "npcname", "")
    local npcname = gui.TextManager:GetText(config_id)
    local desc = ini:ReadString(i - 1, "npcname", "")
    ...
    form.grid_style_info:SetGridText(row, 0, nx_widestr(npcname))
...
function on_btn_buy_click(btn)
  ...
  game_visual:CustomSend(nx_int(7), nx_int(7), nx_int(style_id), form.npcid)
end
function on_grid_style_info_select_row(grid)
  ...
  local gui = nx_value("gui")
  local config_id = ini:ReadString(style_id - 1, "npcname", "")
  local npcname = gui.TextManager:GetText(config_id)
  form.lbl_name.Text = nx_widestr(npcname)
  local describe = ini:ReadString(style_id - 1, "describe", "")
  form.mltbox_describe.HtmlText = nx_widestr(describe)
