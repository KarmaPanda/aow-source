...
local FORM_NAME = "form_stage_main\\form_tongguanjiangli"
function open_form(...)
  ...
  if nx_is_valid(gui) then
    ...
    form.Left = 0
    form.Top = 0
    local delay = nx_int(form.delay)
    form.btn_1.Text = gui.TextManager:GetFormatText("ui_clone_text_9", nx_widestr(delay))
  end
  form.Visible = true
  form:Show()
  form.star = arg[1]
...
function delay_time()
  ...
  local delay = nx_int(form.delay)
  timer:Register(1000, delay, nx_current(), "delay_count_down", form, -1, -1)
end
function delay_count_down(form)
  local gui = nx_value("gui")
  if not nx_is_valid(gui) then
    return
  end
  form.delay = form.delay - 1
  local delay = nx_int(form.delay)
  form.btn_1.Text = gui.TextManager:GetFormatText("ui_clone_text_9", nx_widestr(delay))
  if form.delay <= 0 then
    on_btn_next_click()
    local timer = nx_value(GAME_TIMER)
    timer:UnRegister(nx_current(), "delay_count_down", form)
  end
end
function level_time()
  local form = util_get_form(FORM_NAME)
