...
local FORM_NAME = "form_stage_main\\form_force\\form_force_dmp_smdh"
function open_form()
...
function on_rbtn_con_checked_changed(rbtn)
  ...
  nx_execute("custom_sender", "custom_dmp_schoolmeet", 1)
end
function on_rbtn_damage_checked_changed(rbtn)
  ...
  nx_execute("custom_sender", "custom_dmp_schoolmeet", 2)
end
function server_msg_handle(...)
  ...
  if nx_number(0) == nx_number(sub_msg) then
    ...
    local damage_str = nx_widestr(arg[5])
    set_rank_title(form, 1)
    update_rank_con(form, name_str, gx_str, my_gx_value, damage_str)
...
function set_rank_title(form, index)
  ...
  elseif nx_int(index) == nx_int(2) then
    ...
    form.textgrid_con:SetColTitle(1, nx_widestr(gui.TextManager:GetText("ui_ssg_player_name")))
    form.textgrid_con:SetColTitle(2, nx_widestr(gui.TextManager:GetText("ui_dmp_smdh_02")))
  end
end
function update_rank_con(form, name_str, gx_str, my_gx_value, damage_str)
  local gui = nx_value("gui")
  ...
  if nx_widestr(damage_str) ~= nx_widestr("") then
    damage_tab = util_split_wstring(nx_widestr(damage_str), ",")
  end
  ...
  local damage_count = table.getn(damage_tab)
  if nx_int(damage_count) > nx_int(0) then
    form.groupbox_damage.Visible = true
  end
  if nx_int(damage_count) == nx_int(1) then
    form.lbl_damage.HtmlText = util_format_string("ui_dmp_smdh_damage_1", nx_widestr(damage_tab[1]))
  elseif nx_int(damage_count) == nx_int(2) then
    form.lbl_damage.HtmlText = util_format_string("ui_dmp_smdh_damage_2", nx_widestr(damage_tab[1]), nx_widestr(damage_tab[2]))
  elseif nx_int(damage_count) == nx_int(3) then
    form.lbl_damage.HtmlText = util_format_string("ui_dmp_smdh_damage_3", nx_widestr(damage_tab[1]), nx_widestr(damage_tab[2]), nx_widestr(damage_tab[3]))
  elseif nx_int(damage_count) == nx_int(4) then
    form.lbl_damage.HtmlText = util_format_string("ui_dmp_smdh_damage_4", nx_widestr(damage_tab[1]), nx_widestr(damage_tab[2]), nx_widestr(damage_tab[3]), nx_widestr(damage_tab[4]))
  elseif nx_int(damage_count) == nx_int(5) then
    form.lbl_damage.HtmlText = util_format_string("ui_dmp_smdh_damage_5", nx_widestr(damage_tab[1]), nx_widestr(damage_tab[2]), nx_widestr(damage_tab[3]), nx_widestr(damage_tab[4]), nx_widestr(damage_tab[5]))
  elseif nx_int(damage_count) == nx_int(6) then
    form.lbl_damage.HtmlText = util_format_string("ui_dmp_smdh_damage_6", nx_widestr(damage_tab[1]), nx_widestr(damage_tab[2]), nx_widestr(damage_tab[3]), nx_widestr(damage_tab[4]), nx_widestr(damage_tab[5]), nx_widestr(damage_tab[6]))
  elseif nx_int(damage_count) == nx_int(7) then
    form.lbl_damage.HtmlText = util_format_string("ui_dmp_smdh_damage_7", nx_widestr(damage_tab[1]), nx_widestr(damage_tab[2]), nx_widestr(damage_tab[3]), nx_widestr(damage_tab[4]), nx_widestr(damage_tab[5]), nx_widestr(damage_tab[6]), nx_widestr(damage_tab[7]))
  end
end
