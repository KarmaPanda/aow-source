...
function refresh_form_pos(form)
  ...
  form.groupbox_interactive.AbsLeft = gui.Width * 0.15
  form.groupbox_interactive.AbsTop = form.groupbox_progress.AbsTop - form.groupbox_interactive.Height
...
function change_form_size()
  local form = nx_value("form_stage_main\\form_small_game\\form_game_zhujian_kaifeng")
...
function on_game_end(res)
  ...
  form.groupbox_interactive.Visible = false
  local gui = nx_value("gui")
  ...
  timer:Register(2500, 1, "form_stage_main\\form_small_game\\form_game_zhujian_kaifeng", "auto_close_form", form, -1, -1)
end
function auto_close_form(self)
  local form = self
  if form == nil or not nx_is_valid(form) then
    form = nx_value("form_stage_main\\form_small_game\\form_game_zhujian_kaifeng")
  end
  if nx_is_valid(form) then
    ...
    nx_destroy(form)
    nx_set_value("form_stage_main\\form_small_game\\form_game_zhujian_kaifeng", nx_null())
  end
end
...
function create_path_effect(x, y)
  local form = nx_value("form_stage_main\\form_small_game\\form_game_zhujian_kaifeng")
  if not nx_is_valid(form) then
    return
  end
  ...
  ani_path_pos_list[1].left = form.AbsLeft + form.groupbox_interactive.Left + x
  ani_path_pos_list[1].top = form.AbsTop + form.groupbox_interactive.Top + y
...
function on_ani_end(self)
  ...
  if nx_is_valid(ShijiaEastZhujianGame) then
    ...
    do_action("interact585d")
    play_sound("sound_zhujian_kf_game", "snd\\action\\minigame\\zhujian\\dfsj_zj_kf.wav")
  end
end
