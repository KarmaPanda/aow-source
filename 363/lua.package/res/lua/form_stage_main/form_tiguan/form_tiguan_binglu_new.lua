...
local INI_FILE = "share\\Skill\\BingLu\\BingLuLimit.ini"
function open_form(...)
...
function disable_cant_use_type_ctrl(form)
  ...
  if nx_int(max_binglu_level) <= nx_int(0) then
    max_binglu_level = 999
  end
  local rowcount = client_player:GetRecordRows(BINGLU_RANK_REC)
  if rowcount <= 0 then
    return
  end
  local max_binglu_level_by_step = get_max_level_by_step()
  for i = 1, 11 do
    name = "rbtn_" .. nx_string(i)
    rbtn = nx_custom(form, name)
    if nx_is_valid(rbtn) and nx_int(rbtn.DataSource - 100) >= nx_int(0) and nx_int(rbtn.DataSource - 100) < nx_int(rowcount) then
      ...
      if nx_int(max_binglu_level) <= nx_int(cur_level) or nx_int(max_binglu_level_by_step) > nx_int(0) and nx_int(max_binglu_level_by_step) <= nx_int(cur_level) then
        rbtn.Enabled = false
      end
    end
  end
end
function get_max_level_by_step()
  local switch_manager = nx_value("SwitchManager")
  if not nx_is_valid(switch_manager) then
    return 0
  end
  if not switch_manager:CheckSwitchEnable(855) then
    return 0
  end
  local game_step = switch_manager:GetMainGameStep()
  local IniManager = nx_value("IniManager")
  local ini = IniManager:GetIniDocument(INI_FILE)
  if ini == nil then
    return 0
  end
  local sec_index = ini:FindSectionIndex(nx_string("limit"))
  if sec_index < 0 then
    return 0
  end
  ...
  for i = 1, nx_number(item_count) do
    ...
    if nx_int(game_step) < nx_int(key) then
      return max_lv
    else
      max_lv = ini:GetSectionItemValue(sec_index, i - 1)
    end
  end
  return max_lv
end
