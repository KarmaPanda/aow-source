...
local NEW_EQUIP_TYPE = 11
local NEW_TOOL_TYPE = 12
local NEW_MATERIAL_TYPE = 13
local NEW_TASK_TYPE = 14
...
local BAG_COLS = 6
local FORM_NAME = "form_stage_main\\form_prenewbag"
...
function on_main_form_open(form)
  ...
  init_grid(form.imagegrid_mat, VIEWPORT_NEWMATERIALTOOLBOX, VIEWPORT_ADD_NEWMATERIALTOOLBOX)
  ...
  databinder:AddViewBind(VIEWPORT_NEWMATERIALTOOLBOX, form.imagegrid_mat, FORM_NAME, "on_view_operat")
...
function reset_form_size(form)
  ...
  form.lbl_3.Left = (grid_w + 4 - form.lbl_3.Width) / 2
  form.title.Left = (grid_w + 4 - form.title.Width) / 2 - 3
...
function refresh_item_num(form)
  ...
  mltbox_num:Clear()
  mltbox_num:AddHtmlText(util_format_string("ui_move_out", grid.use_item_num, grid.max_item_num), -1)
...
function refresh_need_money(form)
  ...
  local capital1 = form_pre_bag:CalcMoveMoney(grid)
  if capital1 < 0 then
    return
  end
  ...
  local mltbox_num = form.mltbox_1
  mltbox_num:Clear()
  mltbox_num.HtmlText = htmlTextYinKa
end
function choose_bag_type(type)
  ...
  if type == "1" then
    ...
    form.sel_box = NEW_EQUIP_TYPE
    nx_execute("form_stage_main\\form_prebag", "change_tool_type", MY_EQUIP_TYPE)
  elseif type == "2" then
    form.imagegrid_equip.Visible = true
    form.cur_grid = form.imagegrid_equip
    form.sel_box = NEW_EQUIP_TYPE
    nx_execute("form_stage_main\\form_prebag", "change_tool_type", NEW_EQUIP_TYPE)
  elseif type == "3" then
    form.imagegrid_tool.Visible = true
    form.cur_grid = form.imagegrid_tool
    form.sel_box = NEW_TOOL_TYPE
    nx_execute("form_stage_main\\form_prebag", "change_tool_type", NEW_TOOL_TYPE)
  elseif type == "4" then
    form.imagegrid_mat.Visible = true
    form.cur_grid = form.imagegrid_mat
    form.sel_box = NEW_MATERIAL_TYPE
    nx_execute("form_stage_main\\form_prebag", "change_tool_type", NEW_MATERIAL_TYPE)
  elseif type == "5" then
    form.imagegrid_task.Visible = true
    form.cur_grid = form.imagegrid_task
    form.sel_box = NEW_TASK_TYPE
    nx_execute("form_stage_main\\form_prebag", "change_tool_type", NEW_TASK_TYPE)
  else
    form.imagegrid_equip.Visible = true
    form.cur_grid = form.imagegrid_equip
    form.sel_box = NEW_EQUIP_TYPE
