...
local PRESENT_FORM = "form_stage_main\\form_force_school\\form_wuxian_fete"
...
function on_main_form_open(self)
  ...
  local gui = nx_value("gui")
  self.lettercontent.Text = gui.TextManager:GetText("ui_wxj_jsdd_submit")
end
...
function on_btn_send_click(btn)
  ...
  if form.postbox.persistid ~= -1 and nx_is_valid(form.postbox.persistid) then
    ...
    nx_execute("form_common\\form_confirm", "show_common_text", color_level_dialog, nx_widestr(util_format_string("ui_wxj_jsdd_submit_ok")))
    ...
    game_visual:CustomSend(nx_int(CLIENT_CUSTOMMSG_WUXIAN_FETE), nx_int(CLIENT_SUB_MSG_SUBMIT), nx_string(unique_id), form.postbox.num)
  end
  reset_grid(form)
end
...
function can_present(grid, viewobj)
  ...
  if nx_int(lock_status) > nx_int(0) then
    nx_execute("custom_handler", "custom_sysinfo", 0, 0, 0, 2, nx_string("wxj_jsddguj004"))
    gui.GameHand:ClearHand()
    return false
  end
