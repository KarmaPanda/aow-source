...
function on_main_form_open(form)
  ...
  form.textgrid_player_rank:SetColTitle(3, nx_widestr(gui.TextManager:GetText("ui_lxc_shili")))
  form.textgrid_player_rank:SetColTitle(5, nx_widestr(gui.TextManager:GetText("ui_lxc_nybaowu")))
  form.textgrid_player_rank:SetColTitle(6, nx_widestr(gui.TextManager:GetText("ui_lxc_nyyingxiong")))
  form.textgrid_player_rank:SetColTitle(7, nx_widestr(gui.TextManager:GetText("ui_lxc_nygongcheng")))
  form.textgrid_player_rank:SetColTitle(8, nx_widestr(gui.TextManager:GetText("ui_lxc_nyjifen")))
...
function update_ranking_info(score1, score2, score3, score4, nums1, nums2, nums3, nums4, rows, ...)
  local count = #arg
  local playerInfoSize = 9
  if count < playerInfoSize or math.mod(count, playerInfoSize) ~= 0 then
    return
  end
  local form = nx_value("form_stage_main\\form_world_war\\form_world_war_neiying_ranking")
  ...
  form = nx_value("form_stage_main\\form_world_war\\form_world_war_neiying_ranking")
  ...
  if nx_find_custom(form_world_war_lingxiao_stat, "form_neiying_ranking") and nx_is_valid(form_world_war_lingxiao_stat.form_neiying_ranking) then
    if not form_world_war_lingxiao_stat.rbtn_tab_4.Checked then
      form_world_war_lingxiao_stat.rbtn_tab_4.Checked = true
      form_world_war_lingxiao_stat.WorldWarEnd = true
    end
    form_world_war_lingxiao_stat.form_neiying_ranking.Visible = true
  end
  ...
  if rows == 0 then
    form.textgrid_self_rank:SetGridText(row, 0, nx_widestr(""))
  else
  ...
  local realside = gui.TextManager:GetText(force_desc[arg[4]])
  form.textgrid_self_rank:SetGridText(row, 3, nx_widestr(realside))
  ...
  for i = 1, count / playerInfoSize - 1 do
    ...
    form.textgrid_player_rank:SetGridText(row, 3, nx_widestr(gui.TextManager:GetText(force_desc[arg[i * playerInfoSize + 4]])))
