...
function get_bag_name()
  return "form_stage_main\\form_bag_new"
end
function on_bag_select_changed(grid, index)
  ...
  if is_repair_goods("repair_new_byself", grid, index) then
...
function on_bag_right_click(grid, index)
  ...
  local form_old_bag = nx_value("form_stage_main\\form_bag")
  if nx_is_valid(form_old_bag) and form_old_bag.Visible == true then
    local game_client = nx_value("game_client")
    local item = game_client:GetViewObj(nx_string(VIEW_MAIN), nx_string(view_index))
    if not nx_is_valid(item) then
      return
    end
    local view_id = item:QueryProp("ViewID")
    local dest_view_id = nx_int(0)
    if view_id == 1 then
      dest_view_id = nx_int(VIEWPORT_TOOL)
    elseif view_id == 2 then
      dest_view_id = nx_int(VIEWPORT_EQUIP_TOOL)
    elseif view_id == 3 then
      dest_view_id = nx_int(VIEWPORT_MATERIAL_TOOL)
    elseif view_id == 4 then
      dest_view_id = nx_int(VIEWPORT_TASK_TOOL)
    end
    goods_grid:ViewGridPutToAnotherView(grid, dest_view_id)
    return
  end
...
function is_UseFixItem(grid, index)
  ...
  if nItemType < 100 or 199 < nItemType or nItemType == 190 or nItemType == 191 or 110 <= nItemType and nItemType <= 119 and nItemType ~= 116 or not viewobj:FindProp("MaxHardiness") then
...
function do_onestep_equip(grid, index)
  ...
  if grid.typeid == VIEWPORT_NEWEQUIPTOOLBOX or grid.typeid == VIEWPORT_ADD_NEWEQUIPTOOLBOX then
    ...
    if ret == -1 then
      ...
      local proj = nx_execute("form_stage_main\\form_role_info\\form_onestep_equip", "get_real_projid", form.sel_index)
      local pos = index
...
function confirm_use_item(view_id, view_index)
  ...
  elseif nx_number(item_type) == nx_number(ITEMTYPE_COMPOSE_LLY_NEW) then
    text = gui.TextManager:GetText("ui_lilianyao_01")
  elseif nx_number(item_type) == nx_number(ITEMTYPE_ADD_FCULTY) then
    gui.TextManager:Format_SetIDName("ui_battle_fculty")
    gui.TextManager:Format_AddParam(nx_int(item:QueryProp("UseCount")))
  ...
  elseif nx_number(item_type) == nx_number(ITEMTYPE_FIND_TREASURE) then
    ...
    if item:FindProp("AddBufferList") then
      local str_buffer_list = item:QueryProp("AddBufferList")
      local strlst = util_split_string(str_buffer_list, ";")
      local client_player = game_client:GetPlayer()
      if nx_is_valid(client_player) then
        for i = 1, table.getn(strlst) do
          local result = nx_function("find_replace_buffer_in_rec", client_player, nx_string(strlst[i]), 3)
          if result ~= nil and 1 <= table.getn(result) then
            text = text .. "<br/>" .. nx_string("<font color=\"#eb7272\">") .. nx_string(gui.TextManager:GetText(result[1])) .. nx_string("</font>")
          end
        end
      end
    end
  ...
  nx_execute("form_common\\form_confirm", "show_common_text", dialog, nx_widestr(util_text("ui_buffer_replace")) .. nx_widestr(text))
  dialog:ShowModal()
...
function use_select_pos_item_skill()
  local goods_grid = nx_value("GoodsGrid")
  local decal = nx_value("ground_pick_decal")
  if not nx_is_valid(goods_grid) or not nx_is_valid(decal) then
    return false
  end
  if not (nx_find_custom(goods_grid, "use_skill_item_state") and nx_find_custom(goods_grid, "skill_item_viewid")) or not nx_find_custom(goods_grid, "skill_item_viewindex") then
    return false
  end
  if goods_grid.use_skill_item_state == nil or not goods_grid.use_skill_item_state then
    return false
  end
  local view_id = goods_grid.skill_item_viewid
  local view_index = goods_grid.skill_item_viewindex
  local decal_x = decal.PosX
  local decal_y = decal.PosY
  local decal_z = decal.PosZ
  nx_execute("custom_sender", "custom_send_request_look_school_fight_info", 10, view_id, view_index, decal_x, decal_y, decal_z)
  return true
end
