...
function on_main_form_open(form)
  ...
  if nx_is_valid(databinder) then
    databinder:AddViewBind(VIEWPORT_MATERIAL_TOOL, form, "form_stage_main\\form_life\\form_item_fuse", "on_toolbox_viewport_change")
    databinder:AddViewBind(VIEWPORT_TOOL, form, "form_stage_main\\form_life\\form_item_fuse", "on_toolbox_viewport_change")
  end
  if form.job_id == "treasure" then
    form.rbtn_feibangding.Checked = true
    form.rbtn_bangding.Enabled = false
  else
...
function on_fuse_form_open(job_id)
  local form = util_get_form("form_stage_main\\form_life\\form_item_fuse", true)
  ...
  if nx_string(job_id) == nx_string("sh_ss") or nx_string(job_id) == nx_string("sh_hs") then
    return
  end
  form.job_id = job_id
  local Text_ss = gui.TextManager:GetText("ui_sh_ss")
  form.ipt_in.Text = nx_widestr(Text_ss)
  util_auto_show_hide_form("form_stage_main\\form_life\\form_item_fuse")
end
...
function refresh_tree_list_fuse(form)
  ...
  if nx_string(form.job_id) == nx_string("treasure") then
    form.lbl_tile.Text = gui.TextManager:GetFormatText("treasure_menu_102")
    name_ronghe = gui.TextManager:GetFormatText("ui_bwrh1")
    form.lbl_cailiao.Text = gui.TextManager:GetFormatText("ui_bwrh2")
    form.btn_fuse.Text = gui.TextManager:GetFormatText("ui_bwrh3")
    form.btn_cancel.Text = gui.TextManager:GetFormatText("ui_bwrh5")
  end
  ...
  fuse_root.Font = "font_tree_view"
  local formula_list
  if job_id == nx_string("sh_ss") or job_id == nx_string("sh_hs") then
    local game_client = nx_value("game_client")
    formula_list = get_wx_formula(game_client, job_id)
    if formula_list == nil then
      return
    end
    config_sub_node(form, gui, fuse_root, formula_list, "share\\Item\\fuse_wx.ini")
  else
...
function show_sure_fuse_dialog(form)
end
